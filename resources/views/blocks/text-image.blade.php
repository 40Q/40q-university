<section class="text-image-block">
    <div class="md:px-4 lg:p-0 lg:max-w-temp-container lg:mx-auto">
        <div class="flex flex-col justify-between {{ $imageLeft ? 'lg:flex-row' : 'lg:flex-row-reverse' }} gap-12 md:gap-8 relative pt-16 pb-8">
            @if ($id)
            ...
            <div class="max-md:px-4 lg:w-1/2 relative z-50 text-white flex flex-col items-start md:items-center justify-center lg:justify-start lg:items-start gap-3 lg:gap-4">
                <x-section-header containerClasses="text-start md:text-center lg:text-start lg:w-auto" eyebrowClasses="text-primary-dark max-md:uppercase" headingClasses="text-text max-md:text-3xxl max-md:leading-tight md:tracking-wider max-md:uppercase" textClasses="text-primary-dark lg:pr-20" :showEyebrow="$showEyebrow" :eyebrow="$eyebrow" :showHeading="$showHeading" :heading="$heading" :showText="$showText"
                    :text="$text" />
                @if ($buttonText)
